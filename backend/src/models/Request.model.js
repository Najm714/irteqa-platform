...
import { RequestType } from './RequestType.model.js';
...
const FileSchema = new mongoose.Schema({
  ...
  category: {
    type: String,
    enum: ['request', 'proof', 'delivery', 'modification', 'support', 'final', 'user', 'attachment'],
    default: 'request',
  },
  uploadedAt: {
    type: Date,
...
// ============================================================
// ✅ نموذج المكالمات (CallSchema) - موجود بالفعل
// ============================================================
...
const RequestSchema = new mongoose.Schema({
  ...
  paymentStatus: {
    type: String,
    enum: ['pending', 'submitted', 'verified', 'rejected', 'refunded', 'not_required'],
    default: 'pending',
    index: true,
  },
...
RequestSchema.pre('save', async function(next) {
  ...
  if (!this.requestNumber) {
    try {
      ...
      const count = await RequestModel.countDocuments({ portalId: this.portalId });
...
// إضافة نشاط
RequestSchema.methods.addActivity = function(action, actorId, actorRole, oldValue, newValue, metadata = {}) {
...
// إضافة ملف
RequestSchema.methods.addFile = function(fileId, category, uploadedBy, description = '') {
  ...
  this.addActivity('file_uploaded', uploadedBy, 'user', null, { fileId, category });
  return this;
};

// إضافة رسالة
RequestSchema.methods.addMessage = function(senderId, senderRole, message, attachments = []) {
...
// ✅ تحديث مكالمة بواسطة المعرف (callId)
RequestSchema.methods.updateCallById = function(callId, updateData) {
  ...
  // البحث عن المكالمة بواسطة المعرف
...
RequestSchema.methods.cancelCallById = function(callId, cancelledBy, cancelledRole) {
  if (!this.calls || this.calls.length === 0) {
    throw new Error('No calls found for this request');
  }
...
// ✅ بدء مكالمة
RequestSchema.methods.startCallById = function(callId, startedBy, startedRole) {
  if (!this.calls || this.calls.length === 0) {
    throw new Error('No calls found for this request');
  }
...
// ✅ إنهاء مكالمة
RequestSchema.methods.completeCallById = function(callId, completedBy, completedRole) {
  if (!this.calls || this.calls.length === 0) {
    throw new Error('No calls found for this request');
  }
...
// ✅ الحصول على جميع المكالمات مع معلومات المرسل
...
RequestSchema.methods.markAsRead = function(accountId) {
  ...
  for (const msg of this.messages) {
    if (msg.senderId?.toString() !== accountId) {
      const alreadyRead = msg.readBy?.some(r => r.accountId?.toString() === accountId);
      if (!alreadyRead) {
